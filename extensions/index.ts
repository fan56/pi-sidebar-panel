...
function replayTodos(sessionManager: { getBranch(): Iterable<unknown> }): void {
  for (const entry of sessionManager.getBranch()) {
    ...
    if (d?.tasks && Array.isArray(d.tasks)) todoTasks = d.tasks.map((t) => ({ ...t }));
  }
}
...
function syncTodos(details: { tasks?: TodoTask[] }): void {
  if (details?.tasks && Array.isArray(details.tasks)) {
    ...
    todoTasks = details.tasks.map((t) => ({
      ...t, activeForm: activeFormCache.get(t.id) || t.activeForm || "",
    }));
    // If all tasks are completed or deleted, clear the sidebar todo list
    if (todoTasks.length > 0 && todoTasks.every((t) => t.status === "completed" || t.status === "deleted")) {
      todoTasks = [];
    }
  }
}
...
interface AgentInfo {
  id: string; name: string; status: "running" | "done" | "error"; startMs: number; endMs?: number; replayedAt?: number;
}
...
function onAgentStart(id: string, name: string): void {
  ...
  if (activeAgents.has(id) && activeAgents.get(id)!.status === "running") return;
...
function replayAgents(sessionManager: { getBranch(): Iterable<unknown> }): void {
  const records = new Map<string, AgentInfo>();
  for (const entry of sessionManager.getBranch()) {
    const e = entry as { type?: string; customType?: string; data?: { id?: string; type?: string; description?: string; status?: string; startedAt?: number; completedAt?: number } };
  ...
  for (const [id, info] of records) {
    ...
    if (live && live.status === "running" && live.startMs > (info.startMs || 0)) continue; // live restart is newer
...
interface LspEntry { name: string; command: string; available: boolean; }
let lspEntries: LspEntry[] = [];

function commandExists(cmd: string, cwd: string): boolean {
  try { accessSync(cmd, undefined); return true; } catch {
    for (const dir of (process.env.PATH || "").split(path.delimiter)) {
      try { accessSync(path.join(dir, cmd), undefined); return true; } catch { /* */ }
    }
    return false;
  }
}
  ...
  try {
    ...
    if (config?.servers && typeof config.servers === "object") {
      for (const [name, serverCfg] of Object.entries(config.servers)) {
        ...
        if (cmd) lspEntries.push({ name, command: cmd, available: commandExists(cmd, cwd) });
      }
    }
  } catch { /* */ }
}
...
interface McpEntry { name: string; command: string; running: boolean; }
  ...
  try {
    ...
    if (config?.mcpServers && typeof config.mcpServers === "object") {
      for (const [name, serverCfg] of Object.entries(config.mcpServers)) {
        const cmd = ((serverCfg as Record<string, unknown>).command as string) || "";
        if (cmd) {
          let running = false;
          try {
...
class SidebarComponent implements Component {
  ...
  render(width: number): string[] {
    ...
    lines.push(border("\u2502") + padLine(th.fg("accent", " Sidebar Panel")) + border("\u2502"));
    lines.push(border("\u251c") + border("\u2500".repeat(innerW)) + border("\u2502"));

    // Todos
    const visible = todoTasks.filter((t) => t.status !== "deleted");
    lines.push(border("\u2502") + padLine(th.fg("accent", " Todos")) + border("\u2502"));
    if (visible.length === 0) {
      lines.push(border("\u2502") + padLine(th.fg("dim", "   (empty)")) + border("\u2502"));
    } else {
      for (const t of visible.slice(0, 5)) {
        const icon = t.status === "in_progress" ? th.fg("accent", "\u25cf") :
          t.status === "pending" ? th.fg("dim", "\u25cb") :
            th.fg("success", "\u2713");
        const id = t.status !== "completed" ? th.fg("accent", `#${t.id} `) : th.fg("dim", `#${t.id} `);
        lines.push(border("\u2502") + padLine(` ${icon} ${id}${truncateToWidth(t.activeForm || t.subject, 30)}`) + border("\u2502"));
      }
    }
    lines.push(border("\u2502") + padLine(th.fg("border", "\u2500".repeat(innerW - 2))) + border("\u2502"));

    // Sub-agents
    const MAX_AGENT_ENTRIES = 3;
    const running = [...activeAgents.values()].filter((a) => a.status === "running");
    const recent = [...activeAgents.values()].filter((a) => a.status !== "running");
    ...
    lines.push(border("\u2502") + padLine(th.fg("accent", ` Sub-agents (${activeAgents.size})`)) + border("\u2502"));
    if (running.length === 0 && recent.length === 0) {
      lines.push(border("\u2502") + padLine(th.fg("dim", "   (idle)")) + border("\u2502"));
    } else {
      for (const a of shown) {
        const icon = a.status === "running" ? th.fg("accent", SPINNER[Math.floor(Date.now() / 100) % SPINNER.length]) :
          a.status === "done" ? th.fg("success", "\u2713") : th.fg("error", "\u2717");
        const endMs = a.status === "running" ? Date.now() : (a.endMs ?? a.startMs);
        ...
        const agentText = a.status === "done"
          ? th.fg("dim", th.strikethrough(`${truncateToWidth(a.name, 26)} (${elapsed}s)`))
          : `${truncateToWidth(a.name, 26)} (${elapsed}s)`;
        lines.push(border("\u2502") + padLine(` ${icon} ${agentText}`) + border("\u2502"));
      }
    }
    lines.push(border("\u2502") + padLine(th.fg("border", "\u2500".repeat(innerW - 2))) + border("\u2502"));
    ...
    if (lspEntries.length === 0) {
      lines.push(border("\u2502") + padLine(th.fg("dim", "   (no config)")) + border("\u2502"));
    } else {
      for (const e of lspEntries) {
        const s = e.available ? th.fg("success", "\u2713") : th.fg("warning", "!");
        lines.push(border("\u2502") + padLine(` ${s} ${e.name}`) + border("\u2502"));
      }
    }
    lines.push(border("\u2502") + padLine(th.fg("border", "\u2500".repeat(innerW - 2))) + border("\u2502"));
...
function registerSidebar(pi: ExtensionAPI): void {
  ...
  if (!sidebarListenersRegistered) {
    ...
    pi.events.on("subagents:started", ((payload: { id: string; type?: string; description?: string }) => {
      onAgentStart(payload.id, payload.type || payload.description || "sub-agent");
    ...
    (tui, theme, _keybindings, done) => {
      ...
      sidebarRefreshInterval = setInterval(() => {
        ...
        for (const [id, agent] of activeAgents) {
          if (agent.status !== "running" && agent.endMs &&
              (agent.replayedAt ? now - agent.replayedAt > 300000 : now - agent.endMs > 60000)) {
...
function registerSidebarCommand(pi: ExtensionAPI): void {
  pi.registerCommand("sidebar", {
    ...
    handler: async (args: string, ctx) => {
      ...
      const notify = (msg: string, severity: "success" | "info" | "warning" | "error") =>
      ...
      if (trimmed === "" || trimmed === "toggle") {
        ...
        notify(`Sidebar: ${newVal ? "enabled" : "disabled"}`, newVal ? "success" : "warning");
        return;
      }
